...
class CPlayer: public CGameEntity
{
	...
	public:
		...
		void reset_gamemenu (  )
		{
			...
			pos = respawn;
			shot_at = touched_alien = false;
			curr_anim = &anim[0]; // anima��o de parado para direita
		...
		void reset (  )
		{
			...
			pos = respawn;
			shot_at = touched_alien = false;
			curr_anim = &anim[13]; // anima��o de esperando controle
			left_key = right_key = up_key = down_key = 0;
			jump_key = gun_key = jetpack_key = 0;
			set_state(WAITING);
		}
		...
		bool collect_items (  )
		{
			for (int i(0); i < item.size(); i++)
				for (int j(0); j < c_point.size(); j++)
					...
					{
						// verifica qual item foi coletado e dá a pontua��o
						switch (item[i])
						{
							case 'd':
							case 'e':
			...
			switch (get_state())
			{
				...
				case JETPACK: // usando jet pack
					...
					if (pos.y > limit.y + limit.h) // se passou por baixo da tela
						...
						pos.y = limit.y - map->get_tilesize(); // mantenha a posi��o
